...
Purple Surge lives in a retractable right-hand panel in Navigator and Composer. The persistent tab is 40 points wide; play uses 360 points, and the first-use preview uses 328. In the main window the game temporarily replaces the inspector, preserving the session list. Composer keeps a minimum 640-point task column next to the game. Below 760 points of panel height, the header and task notice compact and the shared game HUD compacts its cards, banner and board, keeping all six rows and the Surge control visible together at 1080 × 740. Hints and secondary controls can scroll; Play online stays in a fixed footer.
...
**Online matches are a primary destination**, following the project owner's clarification. The Puzzles / Online arena selector and prominent Play online button open the existing live arena inside a WKWebView in the same panel. Guest play, human matchmaking, friend challenges, profiles and sign-in remain owned by Purple Surge's live service; Navigator does not implement a competing matchmaking queue or account system. No external browser or game window is launched.
...
The online web view uses its own persistent WebKit profile (an ephemeral profile in demos), HTTPS navigation restricted to Purple Surge and named sign-in providers, no native message handlers, no file loading/upload picker, and no media capture permission. Website cookies belong to the game profile. Online play is loaded only after a player explicitly chooses it. Remote game state never overwrites the local puzzle archive.
