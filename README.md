...
- [Purple Surge inside Codex Navigator — Design](docs/PURPLE-SURGE-DESIGN.md) — agreed in-app game experience, dated 14 September 2026; proposed feature, not implemented.
...
Open **dist/Codex Navigator.app**. The first launch progressively indexes local Codex history; recently updated and selected sessions take priority. Cached sessions appear immediately on subsequent launches.

Requirements: macOS 14+, the Codex desktop app or Codex CLI, and `/usr/bin/python3` (provided by the installed Apple command-line developer tools on this machine).
...
The app is locally ad-hoc signed, not notarized for distribution. All generated files stay in this workspace. Persistent user data normally lives in `~/Library/Application Support/Codex Navigator/navigator.sqlite`.
